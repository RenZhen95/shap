...
class MaskedModel:
    ...
    def _delta_masking_call(self, masks, zero_index=None, batch_size=None):
        ...
        # The masker will accept the sample of interest to explain as self.args
        masked_inputs, varying_rows = self.masker(masks, *self.args)
        # masked_inputs : Inputs from background data, which have been masked with the feature-of-interest's
        # feature value
        # varying_rows  : For each nFeature*2, indicate which background sample has varying values (True)
        num_varying_rows = varying_rows.sum(1)

        # Only considered subset of background samples with variation (I believe ...)
        ...
        # Outputs of the masked dataset (checked for varying inputs)
