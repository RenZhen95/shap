...
class MaskedModel:
    ...
    def __init__(self, model, masker, link, linearize_link, *args):
        ...
        print(f"Masker nRows: {self._masker_rows}")
        print(f"Masker nCols: {self._masker_cols}")
    ...
    def _delta_masking_call(self, masks, zero_index=None, batch_size=None):
        ...
        print("Calling from utils/_masked_model.py::_delta_masking_call 1")
        print(masked_inputs)
        print(masked_inputs[0].shape)
        print(varying_rows)
        print(varying_rows.shape)
        subset_masked_inputs = [arg[varying_rows.reshape(-1)] for arg in masked_inputs]
        print(subset_masked_inputs)
        print(subset_masked_inputs[0].shape)
